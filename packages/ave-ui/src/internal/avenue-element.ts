...
export default class AvenueElement extends LitElement {
   ...
   constructor() {
      ...
      this.customStates.set('ave-defined', true);

      let Self = this.constructor as typeof AvenueElement;
      ...
      if (!isServer) {
         ...
         this.shadowRoot?.prepend(
            document.createComment(
               ` Avenue: https://avenue.bostonuniversity.edu/docs/components/${this.localName.replace('ave-', '')} `,
            ),
         );
      }
   }
   ...
   attributeChangedCallback(name: string, oldValue: string | null, newValue: string | null) {
      if (!this.#hasRecordedInitialProperties) {
         (this.constructor as typeof AvenueElement).elementProperties.forEach(
