...
import { AveInvalidEvent } from '../events/invalid.js';
import { CustomErrorValidator } from './validators/custom-error-validator.js';
import AvenueElement from './avenue-element.js';

export interface Validator<T extends AvenueFormAssociatedElement = AvenueFormAssociatedElement> {
...
export interface AvenueFormControl extends AvenueElement {
   // Form attributes
   name: null | string;
   disabled?: boolean;
...
export class AvenueFormAssociatedElement
   extends AvenueElement
   implements Omit<ElementInternals, 'form' | 'setFormValue'>, AvenueFormControl {
   ...
   emitInvalid = (e: Event) => {
      ...
      this.dispatchEvent(new AveInvalidEvent());
   };
   ...
   protected willUpdate(changedProperties: Parameters<LitElement['willUpdate']>[0]) {
      ...
      if (changedProperties.has('value') || changedProperties.has('disabled') || changedProperties.has('defaultValue')) {
         ...
         // Accounts for the snowflake case on `<ave-select>`
   ...
   get allValidators() {
      const staticValidators = (this.constructor as typeof AvenueFormAssociatedElement).validators || [];
