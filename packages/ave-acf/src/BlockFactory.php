...
final class BlockFactory
{
    ...
    private static function build_component_render_data(
        ...
        $data['props'] = self::adapt_component_props(
            $config,
            $data['props'],
            $is_preview,
            $post_id
    ...
    /**
     * Adapt source-specific values into canonical component contract values.
     *
     * A component contract named "image" resolves to the logical adapter
     * contract "avenue/image". If WordPress has no adapter registered for a
     * contract, the value is already assumed to be canonical and is left alone.
     *
     * @param array<string, mixed> $config
     ...
     * @return array<string, mixed>
     */
    private static function adapt_component_props(
        array $config,
        array $props,
        bool $is_preview,
        $post_id
        ...
        $definitions = $schema['prop_definitions'] ?? null;

        if (!is_array($definitions) || $definitions === []) {
            return $props;
        }

        $field_definitions = self::resolve_field_definitions($config);
        $component_name = isset($config['name']) && is_string($config['name'])
     ...
     * @return array<string, mixed>
     */
    private static function find_field_definition(
        array $definitions,
        string $name
    ): array {
        foreach ($definitions as $definition) {
